...
const CartSelectionScreen: React.FC = () => {
        ...
        try {
          ...
          if (user.customerCode && user.country) {
            ...
            if (response.success && response.data && response.data.length > 0) {
              // Find the specific brand match if multiple returned
  ...
  const handleAddToCart = () => {
    alert(`Added ${qty} of ${item.ArtNr} to cart!`);
    navigate('/search');
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 h-screen overflow-hidden">
      <header className="px-6 py-4 border-b border-blue-100 flex items-center bg-white z-10 shadow-sm">
        ...
        <h2 className="text-xl font-extrabold text-blue-950">Add to Cart</h2>
        {isEnriching && (
      ...
      <main className="flex-1 overflow-y-auto p-4">
        <div className="bg-white rounded-[32px] shadow-xl shadow-blue-900/5 border border-white overflow-hidden">
          <div className="p-6">
            <div className="flex items-start space-x-6">
              <div className="w-24 h-24 bg-slate-50 rounded-[24px] border border-slate-100 flex items-center justify-center shrink-0">
                <img 
              ...
              <div className="flex-1 min-w-0">
                ...
                <h1 className="text-2xl font-black text-slate-900 leading-tight mt-1">{item.ArtNr}</h1>
          ...
          <div className="bg-slate-50/50 p-6 border-t border-slate-100">
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              ...
              <div className="col-span-1 text-right">
                ...
                <span className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase inline-block ${item.Stock === 'Available' ? 'bg-green-600 text-white shadow-lg shadow-green-200' : 'bg-slate-200 text-slate-400'}`}>
                  {item.Stock || 'CHECKING...'}
                </span>
              </div>
              ...
              <div className="col-span-1">
                ...
                <div className="flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-32">
                  <button 
                    onClick={() => setQty(Math.max(1, qty - 1))}
                    className="flex-1 py-2 text-slate-400 hover:bg-slate-50 active:bg-slate-100 font-bold transition-colors"
                  >-</button>
                  <input 
                    type="number" 
                    value={qty}
              ...
              <div className="col-span-1 text-right flex flex-col justify-end">
                ...
                <p className="text-2xl font-black text-blue-900">
                  {totalValue} <span className="text-xs text-blue-300 ml-0.5">{item.Curr}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <footer className="bg-white border-t border-slate-100 p-4 text-center">
        <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Enterprise E-commerce Engine</p>
      </footer>
    </div>
  );
};
